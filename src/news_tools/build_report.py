...
def _highlight_summary(text: str) -> str:
    ...
    # 高亮数字（包括数字+万/千）
    text = re.sub(
        r"(\d+(?:\.\d+)?)(?=\s*(?:个|万|k|⭐|星))", r'<span class="hl">\1</span>', text
    )
    text = re.sub(
        r'(?<![.<>"])(\d+)(?![.\d]*<\/[^>]+>)(?!\s*\.\d)',
    ...
    text = re.sub(
        r"(?<=涵盖\s)(.*?)(?=\s*等领域)",
        lambda m: re.sub(r"([^·\s][^·]*?)(?=·|$)", r"<strong>\1</strong>", m.group(1)),
        text,
    )
    return text
...
def _download_avatars(
    repos: list[dict], avatar_dir: Path, proxies: Optional[dict] = None
) -> None:
...
def build_report(
    ...
    try:
        ...
        if (
            ...
            report_date = datetime(
                int(parts[0]), int(parts[1]), int(parts[2]), tzinfo=CST
            )
        elif len(parts) >= 2 and parts[0].isdigit() and parts[1].isdigit():
            # YYYY-WW format (year-week)
            iso_date = date.fromisocalendar(int(parts[0]), int(parts[1]), 1)
            report_date = datetime(
                iso_date.year, iso_date.month, iso_date.day, tzinfo=CST
            )
        else:
    ...
    week_info = (
        f"{report_date.year} 年第 {week_num} 周 / {report_date.strftime('%Y-%m-%d')}"
    )
    ...
    # ── 右侧索引（新版：指向连续滚动 repo） ──────────────
    index_items = ""
    index_items += f'      <a class="index-item" href="#gh-cover"><span class="idx-num">✦</span> 排行榜</a>\n'
    index_items += '      <div class="index-divider"></div>\n'
    for idx, r in enumerate(repos):
        rank = idx + 1
        index_items += f'      <a class="index-item" href="#repo-{rank:02d}"><span class="idx-num">#{rank:02d}</span> {r["name"]}</a>\n'
...
def main() -> None:
    ...
    parser.add_argument(
        "-o", "--output-dir", default=None, help="输出目录（默认与 JSON 同目录）"
    )
    parser.add_argument(
        "--avatar-dir", default="avatar", help="头像子目录名（默认 avatar）"
    )
